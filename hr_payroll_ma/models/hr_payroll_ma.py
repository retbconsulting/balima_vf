...
class hr_payroll_ma(models.Model):
    ...
    def generate_employees(self):
        ...
        for rec in self:
            ...
            obj_contract = self.env['hr.version']
            ...
            for e in emp :
                ...
                if contract_ids and employe.company_id == rec.company_id:
                    ...
                    conge_paid = self.env['hr.leave.type'].search([('name', 'like', 'Congés payés'), ('company_id', '=', e.company_id.id)], limit=1)
                    ...
                    line = {
                            ...
                            'conges_payes': number_days_not_working,
    ...
    def compute_fp(self):
        for record in self:
            bulletins = self.env['hr.payroll_ma.bulletin'].search([('id_payroll_ma', '=', record.id)])
...
class hr_payroll_ma_bulletin(models.Model):
    ...
    employee_contract_id = fields.Many2one('hr.version', string=u'Contrat de travail', required=True)
    ...
    real_fp = fields.Float(string='FP', size=64, digits=(16, 2))
    ...
    def compute_fp(self):
        for bulletin in self:
            salaire_brute_imposable_ir = bulletin.salaire_brute_imposable_ir
            fraispro = 0
            if salaire_brute_imposable_ir <= 6500:
                fraispro = salaire_brute_imposable_ir * 0.35
            else:
                fraispro_base = salaire_brute_imposable_ir * 0.25
                if fraispro_base < 2917:
                    fraispro = fraispro_base
                else:
                    fraispro = (2917 * (bulletin.working_days + bulletin.conges_payes)) / 26
            bulletin.real_fp = fraispro
    ...
    def get_igr_2(self, montant,rubriques, cotisations, css=False):
        ...
        for rec in self:
            ...
            if not bulletin.employee_contract_id.ir:
                ...
                logement = bulletin.employee_id.logement
                frais_pro_reelle = 0
                ### TO IMP : The hardcoded number just to ensure the right calculation (For urgency). New commit will be pushed after calculation validate
                if montant <= 6500:
                    #fraispro = montant * dictionnaire.fraispro / 100
                    ...
                    if fraispro < 2917:
                        ...
                        salaire_net_imposable = montant - (2917 * (bulletin.working_days + bulletin.conges_payes)) / 26 - cotisations
...
class hr_ligne_rubrique(models.Model):
    ...
    id_contract = fields.Many2one('hr.version', string='Ref Contrat', ondelete='cascade', select=True)
