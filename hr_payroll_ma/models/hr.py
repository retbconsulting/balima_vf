    ...
    def _check_param_logement_social(self):
        if self.type_logement == 'social':
            ...
            if self.superficie_logement > dictionnaire['superficie_max_logement_social']:
                raise ValidationError("Attention La superficie indiquée n'est pas conforme aux normes du logement social !!!")

            # On vérifie le prix d'acquisition
            if self.prix_acquisition_logement > dictionnaire['prix_achat_max_logement_social']:
                raise ValidationError("Attention Le prix d'acquisition n'est pas conforme aux normes du logement social !!!")
...
class hr_contract(models.Model):
    ...
    def _check_unicite_contrat(self):
        contrat_ids = self.env['hr.version'].search([('employee_id','=',self.employee_id.id),('actif','=',True)])
        print ("contrats"), len(contrat_ids)
        if len(contrat_ids) > 1:
            raise ValidationError("Attention Plusieurs contrats actifs pour cet employé!!!")
