...
class e_bds(models.TransientModel):
    ...
    def generate(self):
        ...
        bulletin_salaire = self.env['hr.payroll_ma.bulletin']
        ...
        for rec in self:
            ...
            e_bds_sortant_id = e_bds_sortant.search([('name.name', '=', this.period_id.name)])
            file_content_decoded = base64.b64decode(this.file_import).decode('utf-8')
            #file_content_decoded = base64.decodestring(this.file_import)
            ...
            for emp in data[2:number+2]:
                salaire_reel = 0
                N_SALAIRE_Plaf = ''
                S_CTR = ''
                bulletin_id = bulletin_salaire.search([('employee_id.ssnid', '=', emp[16:25]),('period_id.name', '=', this.period_id.name)])
                
                if bulletin_id:
                    
                    bulletin_id[0].normal = True
                    nombre_salaries += 1
                    total_matriculation += int(emp[16:25])
                    #salaire_reel = bulletin_id[0].salaire_brute_imposable
                    salaire_reel = bulletin_id[0].mapped('salary_line_ids').filtered(lambda r:r.name == 'Participation AMO').base
                    working_days = bulletin_id[0].working_days + bulletin_id[0].conges_payes
                    ...
                    if e_bds_sortant_id:
                        e_bds_sortant_line_id = e_bds_sortant_line.search([('e_bds_sortant_id', '=', e_bds_sortant_id[0].id),('employee_id.ssnid', '=', emp[16:25])],limit=1)
                        if e_bds_sortant_line_id:
                            ...
                            if L_situation == 'SO':
                                ...
                                N_SALAIRE_Plaf ="".rjust(9,'0')
                                total_N_SALAIRE_Plaf -= int(sal_plaf)
                                salaire_reel_str = "".rjust(13,'0')
                                total_salaire_reel -= int(salaire_reel*100)
                                situation_chiffre = 7
                            else:
                                situation_chiffre = 8
                    ...
                    S_CTR_sortant = 0
                    if e_bds_sortant_id:
                        e_bds_sortant_line_id = e_bds_sortant_line.search([('e_bds_sortant_id', '=', e_bds_sortant_id[0].id),('employee_id.ssnid', '=', emp[16:25])])
                        if e_bds_sortant_line_id:
                            nombre_salaries += 1
                            total_matriculation += int(emp[16:25])
            ...
            output += B03 + '\n'
            bulletin_ids = bulletin_salaire.search([('normal', '=', False),('period_id.name', '=', this.period_id.name)])
            ...
            N_T_Ctr_entrants = 0
            for entrant in bulletin_ids:
                salaire_entrant = ""
                N_Num_Assure = entrant.employee_id.ssnid
                L_Nom_Prenom = entrant.employee_id.name
                L_Num_CIN = entrant.employee_id.cin
                N_Nbr_Jours = int(str(entrant.working_days + entrant.conges_payes).replace('.', '')[:-1])
                N_Sal_Reel = int(entrant.mapped('salary_line_ids').filtered(lambda r:r.name == 'Participation AMO').base*100)
                N_Sal_Plaf = 0
                ...
                salaire_entrant = "B04" + l2[3:16] + str(N_Num_Assure).rjust(9,'0') + str(L_Nom_Prenom).rjust(60) + str(L_Num_CIN).rjust(8," ") + str(N_Nbr_Jours).rjust(2,'0') + str(N_Sal_Reel).rjust(13,'0') + str(N_Sal_Plaf).rjust(9,'0') + str(S_Ctr).rjust(19,'0') + "".rjust(124)
                output += salaire_entrant + '\n'

            if not bulletin_ids:
                salaire_entrant = "B04" + l2[3:16] + str('').rjust(9) + str('').rjust(60) + str('').rjust(8," ") + str(0).rjust(2,'0') + str(0).rjust(13,'0') + str(0).rjust(9,'0') + str(0).rjust(19,'0') + "".rjust(124)
                output += salaire_entrant + '\n'
            ...
            output += B06 + '\n'
            out = base64.encodestring(output.encode())
            #out = base64.b64encode(output).encode('utf-8')
            this.state = 'get'
            this.file_export = out
